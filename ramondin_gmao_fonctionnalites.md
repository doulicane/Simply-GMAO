...
## GMAO Ramondin — Capsules Aluminium
...
17. **ACT-017** — Export PDF historique équipement : sélectionner période, générer PDF avec logo Ramondin, fiche identité équipement, liste BT chronologique, temps total d'arrêt, coûts estimés.
...
- Stockage fichiers en local sur le serveur (path `/var/gmao-ramondin/uploads/`). Pas de CDN cloud.
...
#### Contraintes techniques spécifiques
- Stockage local : `/var/gmao-ramondin/documents/` avec sous-répertoires par équipement.
...
14. **RPT-014** — Export PDF fiche équipement + historique : génération côté serveur (librairie PDF : Puppeteer, WeasyPrint, ou jsPDF), mise en page professionnelle avec logo Ramondin.
...
1. **OPP-001** — Écran d'accueil opérateur : grand bouton "Scanner ma machine" + bouton "Voir mes demandes". Design minimaliste, pas de menu latéral. Logo Ramondin.
...
```javascript
// Structure des object stores IndexedDB (nom: "gmao_ramondin_db", version: 1)
