# Architecture Technique — GMAO Simply GMAO
...
- Le volume Docker `/data/uploads` est monté sur un dossier du host (ex: `/opt/simply-gmao-gmao/uploads`)
...
services:
  ...
  frontend:
    image: simply-gmao/gmao-frontend:${VERSION:-latest}
    build: ./frontend
    environment:
      - VITE_API_URL=/api/v1

  api:
    image: simply-gmao/gmao-api:${VERSION:-latest}
    build: ./api
    environment:
      - DATABASE_URL=postgresql://gmao:password@postgres:5432/simply-gmao_gmao
...
./scripts/generate-ssl.sh gmao.simply-gmao.local
...
curl -k https://gmao.simply-gmao.local/api/v1/health
```
...
| **Données industrielles** | Hébergées chez un tiers (AWS US, Azure...) | Restent dans l'enceinte Simply GMAO |
...
| Solution | Inconvénient majeur pour Simply GMAO |
|----------|-----------------------------------|
...
**Verdict** : Les solutions open source existantes ne couvrent pas les besoins spécifiques de Simply GMAO (mobile-first, offline, ATEX, compteurs) sans un travail de développement et de fork considérable. Il est plus efficace de développer une solution sur mesure avec une stack moderne.
...
*Document produit pour le projet GMAO Simply GMAO — Hébergement local, PWA offline-first, stack moderne Dockerisée.*
